	...
	var ConfigTemplete = {
		...
		t_side:{//父菜单
			tagName:'li',
			currentStyle: "open",
			temp:'<a href="javascript:;11"><i></i>%1</a>'
		},
		side:{//子菜单
			currentStyle: "focus",
			tagName:'ul',
		...
		var getList = function(item,defaultKey){
			...
			for(var i = 0,len = item.Childs.length; i < len; i++){
				...
				li.bind("click",{p:getParent(obj),c:obj},function(e){
					//display(e.data.p,e.data.c.Key);//去掉后 左侧菜单不会自动滚动到顶部
	...
	var TopMenuControl = (function(){
		...
		var bindEvents = function(obj){
			obj.tab.click(function(e){
				var aObj = insertItem(obj.key,MenuData[obj.key]);
		...
		var deleteItem = function(key){
			if(_cache[key]){
				_cache[key].tab.remove();
				_cache[key].content.remove();
				if(_cache[key].actived){
					var oldKey;
					var isexit = false;
					for(var i = 0,len = _tabBox.children.length; i < len; i++){
					...
					if(oldKey == undefined){
						oldKey = TopMenuControl.DefaultKey;
						var aObj = insertItem(oldKey,MenuData[oldKey]);
						if(TopMenuControl.ClickCallBack){
							TopMenuControl.ClickCallBack(aObj);
						}
					}
	...
	var MainAPI = (function(){
		var Return = {
			SelectTab: function(url){
				...
				if(aKey != undefined){
					TopMenuControl.Add(aKey,MenuData[aKey]);
				}
			},
			FullScreen:function(){
